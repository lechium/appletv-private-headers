/**
 * This header is generated by class-dump-z 0.2a.
 * class-dump-z is Copyright (C) 2009 by KennyTM~, licensed under GPLv3.
 *
 * Source: /System/Library/PrivateFrameworks/YouTubeATV.framework/YouTubeATV
 */

#import "YTXMLHTTPRequest.h"

@class YTUserProfile;
@protocol YTUserProfileRequestDelegate;

@interface YTUserProfileRequest : YTXMLHTTPRequest {
	id<YTUserProfileRequestDelegate> _delegate;	// 156 = 0x9c
	YTUserProfile *_profile;	// 160 = 0xa0
}
- (id)initWithDelegate:(id)delegate;	// 0x33d16b71
- (void)dealloc;	// 0x33d16c01
- (void)didParseData;	// 0x33d16dd1
- (void)failWithError:(id)error;	// 0x33d16e3d
- (int)parseData:(id)data;	// 0x33d16d4d
- (void)requestProfileForCurrentUser;	// 0x33d16c4d
- (void)requestProfileForUsername:(id)username;	// 0x33d16cc5
@end

