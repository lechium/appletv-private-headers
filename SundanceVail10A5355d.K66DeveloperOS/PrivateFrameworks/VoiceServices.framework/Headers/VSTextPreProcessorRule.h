/**
 * This header is generated by class-dump-z 0.2a.
 * class-dump-z is Copyright (C) 2009 by KennyTM~, licensed under GPLv3.
 *
 * Source: /System/Library/PrivateFrameworks/VoiceServices.framework/VoiceServices
 */

#import <NSObject.h> // Unknown library
#import "VoiceServices-Structs.h"

@class NSString;

@interface VSTextPreProcessorRule : NSObject {
	NSString *_matchPattern;	// 4 = 0x4
	NSString *_replacement;	// 8 = 0x8
	BOOL _caseSensitive;	// 12 = 0xc
	BOOL _eatPunctuation;	// 13 = 0xd
}
- (id)initWithDictionaryRepresentation:(id)dictionaryRepresentation;	// 0x361b2c15
- (void)dealloc;	// 0x361b2d4d
- (id)matchedString:(id)string forTokenInRange:(XXStruct_K5nmsA *)range;	// 0x361b2db1
@end

