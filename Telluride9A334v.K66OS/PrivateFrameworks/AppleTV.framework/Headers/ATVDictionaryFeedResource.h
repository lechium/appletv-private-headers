/**
 * This header is generated by class-dump-z 0.2a.
 * class-dump-z is Copyright (C) 2009 by KennyTM~, licensed under GPLv3.
 *
 * Source: /System/Library/PrivateFrameworks/AppleTV.framework/AppleTV
 */

#import "ATVReadWriteFeedResource.h"
#import <NSObject.h> // Unknown library

@class NSLock, NSMutableDictionary;

__attribute__((visibility("hidden")))
@interface ATVDictionaryFeedResource : NSObject <ATVReadWriteFeedResource> {
@private
	NSMutableDictionary *_prefs;	// 4 = 0x4
	NSLock *_prefsLock;	// 8 = 0x8
}
- (id)init;	// 0x331a76f9
- (void)clearFeedResources;	// 0x331a78e9
- (void)dealloc;	// 0x331a7775
- (id)feedResourceNamed:(id)named;	// 0x331a77d5
- (void)setFeedResource:(id)resource named:(id)named;	// 0x331a7861
@end

